...
import { useUpdateExperienceMutation } from '../../services/ExperienceApi';
import axios from 'axios';


export const ExperUpdate = ({ setUpdateModal, id }) => {
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        try {
          const response = await axios.get(`http://127.0.0.1:8000/api/experiences/${id}/`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await updateExperience({ id, formData });
      ...
      if (res.data) {
        ...
        // Assuming you have a function named storeToken
        ...
        <div className="relative w-4/5 sm mx-auto bg-white my-10">
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            <div className="flex items-start justify-between p-2 border-b bg-light-gray border-solid border-gray-300 rounded-t ">
              <h3 className="text-3xl font-semibold bg-gradient-to-r from-fuchsia-600 via-violet-600 to-indigo-600 bg-clip-text text-transparent">Update Experience</h3>
            ...
            <div className="relative p-6  flex-auto h-[500px] overflow-auto">
              <form onSubmit={handleSubmit}>
                ...
                <div className="relative flex  flex-col mt-4  justify-center">
                  <button className="bg-gradient-to-r from-fuchsia-600 to-purple-600 w-max mx-auto text-white font-semibold px-10 py-2 rounded-2xl hover:shadow-sm transition-all duration-500">Update Experience</button>
                </div>
              </form>
            </div>
          </div>
