...
import { EducUpdate } from "./EducUpdate";

import { useAllEducationQuery } from "../../services/EducationApi";
import { EducDelete } from "./EducDelete";


export const EducInfo = () => {
  const { data: educations, isLoading, isError, error, isSuccess } = useAllEducationQuery();
    ...
    <div id="education">
      <div className="mx-auto bg-white h-auto pt-20 mt-12 pb-4  shadow-2xl rounded-md w-11/12">
        <div className="flex justify-between w-full p-4  ">
          <h1 className="ml-3 text-4xl underline-offset-8 underline font-bold ">Education</h1>
          <div>
            ...
            {showModal && <EduAdd setShowModal={setShowModal} />}
          </div>
        </div>
              ...
              <tbody>
                {educations?.map((educ) => (
                  <tr key={educ.id}>
                    ...
                    <td className="px-5 py-2 border-b border-gray-200 bg-white ">
                      <p className="text-black whitespace-no-wrap">{educ.institute}</p>
                    </td>
                    <td className="px-5 py-2 border-b border-gray-200 bg-white ">
                      <div className="flex gap-4">
                        <Tooltip title="Edit" onClick={() => [setId(educ.id), setUpdateModal(true)]}>
                          <IconButton >
                            <EditIcon className="text-green-500" />
                          </IconButton>
                        </Tooltip>
                        {UpdateModel && <EducUpdate id={id} setUpdateModal={setUpdateModal} />}
                        <Tooltip title="Delete">
                          <IconButton onClick={() => [setId(educ.id), setDeleteModal(true)]}>
                            <DeleteIcon />
                          </IconButton>
                        </Tooltip>
                        {DeleteModal && <EducDelete id={id} setDeleteModal={setDeleteModal} />}
