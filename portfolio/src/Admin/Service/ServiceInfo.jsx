...
import { useAllServicesQuery } from "../../services/ServiceApi";
import { ServiceUpdate } from "./ServiceUpdate";
import { ServiceDelete } from "./ServiceDelete";
import { ServiceAdd } from "./ServiceAdd";

export const ServiceInfo = () => {
  const { data: services, isLoading, isError, error, isSuccess } = useAllServicesQuery();
    ...
    <div id="service">
      <div className="mx-auto bg-white h-auto pt-20 mt-12 pb-4 shadow-2xl rounded-md w-11/12">
        <div className="flex justify-between w-full p-4  ">
          <h1 className="ml-3 text-4xl underline-offset-8 underline font-bold ">Service</h1>
          <div>
            ...
            {showModal && <ServiceAdd setShowModal={setShowModal} />}
          </div>
        </div>
              ...
              <tbody>
                {services?.map((service) => (
                  <tr key={service.id}>
                    ...
                    <td className="px-5 py-2 border-b border-gray-200 bg-white ">
                      <p className="text-black whitespace-no-wrap">{service.description}</p>
                    </td>
                    <td className="px-5 py-2 border-b border-gray-200 bg-white ">
                      <div className="flex gap-4">
                        <Tooltip title="Edit" onClick={() => [setId(service.id), setUpdateModal(true)]}>
                          <IconButton >
                            <EditIcon className="text-green-500" />
                          </IconButton>
                        </Tooltip>
                        {UpdateModel && <ServiceUpdate id={id} setUpdateModal={setUpdateModal} />}
                        <Tooltip title="Delete">
                          <IconButton onClick={() => [setId(service.id), setDeleteModal(true)]}>
                            <DeleteIcon />
                          </IconButton>
                        </Tooltip>
                        {DeleteModal && <ServiceDelete id={id} setDeleteModal={setDeleteModal} />}
